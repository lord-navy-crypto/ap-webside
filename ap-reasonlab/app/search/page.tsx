...
export default function SearchPage() {
  ...
  const results = useMemo<SearchResult[]>(() => {
    const staticResults: SearchResult[] = [
      ...
      ...formulas.map((item) => ({ id: item.id, title: item.name, subject: item.subject, type: "formula", detail: item.content || `${item.expression} · ${item.unit}`, href: `/formulas?subject=${encodeURIComponent(item.subject)}` })),
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold">Search all AP content</h1>
        <p className="mt-2 text-slate-600">Find concepts, formulas, practice sets, and published manager content across every subject.</p>
      </div>
      <section className="card grid gap-3 md:grid-cols-[1fr_12rem]">
        <input autoFocus type="search" className="input" placeholder="Search at least two characters…" value={query} onChange={(event) => setQuery(event.target.value)} />
      ...
      <section className="space-y-3">
        ...
        {query.trim().length >= 2 && results.length === 0 && (
          <div className="card text-sm text-slate-500">No matching content.</div>
        )}
      </section>
    </div>
  );
